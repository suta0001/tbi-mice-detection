...
train_data_path = 'data/epochs_{}c'.format(str(num_classes))
test_data_path = 'data/epochs_novl_{}c'.format(str(num_classes))
...
for fold in range(len(dataset_folds)):
    ...
    train_set = dataset_folds[fold][0:8]
    test_set = dataset_folds[fold][8:]

    # train the model
    train_gen = dg.DataGenerator(train_data_path, file_template, train_set,
                                 'train', batch_size, num_classes,
                                 decimate=decimate_factor,
                                 test_percent=20,
                                 overlap=True)
    val_gen = dg.DataGenerator(train_data_path, file_template, train_set,
                               'train', batch_size, num_classes,
                               decimate=decimate_factor,
                               test_percent=20,
                               overlap=True)
    ...
    test_gen = dg.DataGenerator(test_data_path, file_template, test_set,
