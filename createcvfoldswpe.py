...
wpe_template = '{0}_{1}_ew{2}.csv'
wpe_common_labels = [eeg_source, str(eeg_epoch_width_in_s)]
epoch_template = '{0}_ew{1}.csv'
epoch_common_labels = [str(eeg_epoch_width_in_s)]
cv_template = '{0}_{1}_ew{2}_f{3}_wpe.csv'
...
for fold, dataset_fold in enumerate(dataset_folds):
    ...
    for i in range(0, 7):
        ...
        input_filename = wpe_template.format(dataset_fold[i] + '_BL5',
                                             *wpe_common_labels)
        sd.read_data(wpe_path + input_filename, wpe_epochs)
        eeg_epochs = [wpe_epochs[i] for i in range(len(wpe_epochs))]
