...
for fold in range(10):
    ...
    sd.read_data(cv_path + input_filename, total_epochs)
    input_filename = template.format('train_labels', *common_labels)
    sd.read_data(cv_path + input_filename, total_labels)
    ...
    for i in range(len(total_epochs)):
        if i % 5 == 0:
            test_index = i + np.random.randint(0, 5) 
        if i == test_index:
            test_epochs.append(total_epochs[i])
            test_labels.append(total_labels[i])
        else:
            train_epochs.append(total_epochs[i])
